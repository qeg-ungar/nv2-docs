...
m\ :sub:`s` = ±1 state, reducing fluorescence (negative contrast). The resonance linewidth is
limited by power broadening and T₂* in the CW regime.
...
Set the signal generator (or OPX) to sweep MW frequency across the expected resonance.
...
.. list-table::
   ...
   * - Sweep center
     - 2.87 GHz
     - Zero-field; shift with bias field
   * - Sweep span
     - 100–300 MHz
     - Wider if field is applied
   * - MW power at antenna
     -
     - dBm
...
.. list-table::
   ...
   * - Contrast
     - > 5%
     - < 1%
     - Low MW power, misaligned MW
   * - Linewidth
     - 5–20 MHz
     - > 50 MHz
     - Power broadened, bad sample
   * - Resonance frequency
     - 2.87 ± field shift
     - No dip visible
     - MW not reaching sample

Common Issues
-------------

- **No ODMR dip visible** — check MW power at PCB (use power meter), verify antenna coupling,
...
*This record serves as the baseline for tracking instrument health over time.*
